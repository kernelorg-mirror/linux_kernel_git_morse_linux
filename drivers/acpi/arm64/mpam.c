...
static bool parse_irq_common(struct platform_device *pdev, int intid,
			     u32 flags, int *irq, u32 processor_container_uid)
{
	int sense;
				 ...
				 struct resource *res, int *res_idx)
{
	...
	if (flags & ACPI_MPAM_MSC_IRQ_AFFINITY_VALID &&
		...
		aff = GLOBAL_AFFINITY;
	if (parse_irq_common(pdev, tbl_msc->overflow_interrupt, flags, &irq, aff))
	...
	if (flags & ACPI_MPAM_MSC_IRQ_AFFINITY_VALID &&
		...
		aff = GLOBAL_AFFINITY;
	if (parse_irq_common(pdev, tbl_msc->error_interrupt, flags, &irq, aff))
...
static int __init acpi_mpam_parse(void)
{
	...
	while (table_offset < table_end) {
		 ...
		 * wide supported partid and pmg ranges. This avoids this MSC
		 * truncating the partids and creating a screaming error
		 * interrupt.
		 */
