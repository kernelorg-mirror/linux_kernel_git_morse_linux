...
static int __topology_l3_equivalent(int cpu, cpumask_var_t tmp_cpumask)
{
	int err;
				  ...
				  cpumask_var_t tmp_cpumask)
{
	 ...
	 * The topology matches if each victim:component has a corresponding
	 * L3:component with the same affinity mask.
	 * These lists/masks are computed from firmware tables so don't change
	 * at runtime.
	 */
	list_for_each_entry(victim_iter, &victim->components, class_list) {
		if (cpumask_empty(&victim_iter->affinity)) {
			pr_debug("%s: CPU-less component can't match L3!\n", __func__);
			return false;
		}
		...
		err = __topology_l3_equivalent(cpu, tmp_cpumask);
		if (err) {
		...
		cpumask_xor(tmp_cpumask, tmp_cpumask, &victim_iter->affinity);
...
static void mpam_resctrl_pick_caches(void)
{
	...
	list_for_each_entry_rcu(class, &mpam_classes, classes_list) {
		...
		if (!has_cpor && !has_cmax) {
			pr_debug("pick_caches: Cache misses CPOR or CMAX\n");
			continue;
		}

		if (!cpumask_equal(&class->affinity, cpu_possible_mask)) {
			pr_debug("pick_caches: Class has missing CPUs\n");
			pr_debug("pick_caches: mask %*pb != %*pb\n",
				      ...
				      enum resctrl_event_id type)
{
	...
	/*
	 * If there are no MPAM resources on L3, force it into existance.
	...
	if (!res->class) {
		...
		res->class = mpam_resctrl_counters[type];
	}

	/* Called multiple times!, once per event type */
	if (exposed_mon_capable) {
		l3->mon_capable = true;
		pr_err("l3->mon_capable = true;\n");
			    ...
			    u32 closid, enum resctrl_conf_type t, u32 cfg_val)
{
	...
	* the same component is exposed multiple times through resctrl.
	*/
	cfg = dom->comp->cfg[partid];
