...
static void mpam_resctrl_pick_mba(void)
{
	...
	list_for_each_entry(class, &mpam_classes, classes_list) {
		 ...
		 * RDT has cmdline options that cover these cases...
		 */
		if (!mpam_has_feature(mpam_feat_msmon_mbwu, class->features))
			pr_warn("MBA resource may become unsupported");
...
static void mpam_resctrl_pick_caches(void)
{
	...
	list_for_each_entry(class, &mpam_classes, classes_list) {
		/*
		 * Resctrl expects to be able to point smp_call_function_any()
		 * at the domain list. If the domain is made up of multiple
		 * devices we'd need to IPI a set of CPUs to do the work.
		 * Only consider classes that have 1 device per component.
		 */
		if (class->max_devices != 1)
			continue;
			     ...
			     hw_closid_t hw_closid, u32 *value)
{
	...
	lockdep_assert_cpus_held();	// lest @d disappear
