 ...
 */
u16 mpam_partid_max;
#define MPAM_PARTID_NUM		(mpam_partid_max + 1)
u8 mpam_pmg_max;
...
static void mpam_reset_msc_bitmap(struct mpam_msc *msc, u16 reg, u16 wd)
{
	...
	for (i = 0; i < num_words - 1; i++, reg += sizeof(u32))
		__mpam_write_reg(msc, reg, ~0);
	 ...
	 */
	msb = (wd - 1) % 32;
	__mpam_write_reg(msc, reg, GENMASK(msb, 0));
}
...
static int __allocate_component_cfg(struct mpam_component *comp)
{
	...
	comp->cfg = kcalloc(MPAM_PARTID_NUM, sizeof(*comp->cfg), GFP_KERNEL);
	if (!comp->cfg)
		return -ENOMEM;
			    ...
			    unsigned long cmd, void *v)
{
	...
	switch (cmd) {
        case CPU_PM_EXIT:
        	 ...
        	 * may not match arm64_mpam_current.
        	 */
