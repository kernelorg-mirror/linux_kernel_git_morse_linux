...
static u64 mpam_msc_read_esr(struct mpam_msc *msc)
{
	...
	esr_low = mpam_read_reg(msc, MPAMF_ESR);
	if (msc->has_extd_esr)
		esr_high = mpam_read_reg(msc, MPAMF_ESR + 4);
...
/* synchronise_rcu() before freeing ris */
 ...
 * synchronise_rcu() before freeing msc.
 */
...
int mpam_ris_get_affinity(struct mpam_msc *msc, cpumask_t *affinity,
			  enum mpam_class_types type, struct mpam_class *class,
			  struct mpam_component *comp)
{
	int err;
...
int mpam_ris_create_locked(struct mpam_msc *msc, u8 ris_idx,
...
static void mpam_ris_hw_probe(struct mpam_msc_ris *ris)
{
	...
	if (FIELD_GET(MPAMF_IDR_HAS_CCAP_PART, ris->idr)) {
		u32 ccap_features = mpam_read_reg(msc, MPAMF_CCAP_IDR);
	...
	if (FIELD_GET(MPAMF_IDR_HAS_PRI_PART, ris->idr)) {
		u32 pri_features = mpam_read_reg(msc, MPAMF_PRI_IDR);
	...
	if (FIELD_GET(MPAMF_IDR_HAS_PARTID_NRW, ris->idr) &&
		...
		u32 nrwidr = mpam_read_reg(msc, MPAMF_PARTID_NRW_IDR);
				      ...
				      struct mpam_config *cfg)
{
	...
	if (mpam_has_feature(mpam_feat_cpor_part, rprops)) {
		if (mpam_has_feature(mpam_feat_cpor_part, cfg))
			mpam_write_reg(msc, MPAMCFG_CPBM, cfg->cpbm);
		else
	...
	if (mpam_has_feature(mpam_feat_mbw_part, rprops)) {
		if (mpam_has_feature(mpam_feat_mbw_part, cfg))
			mpam_write_reg(msc, MPAMCFG_MBW_PBM, cfg->mbw_pbm);
		else
...
static void mpam_reprogram_ris(void *_arg)
{
	unsigned long flags;
...
static void mpam_enable_msc_ecr(void *_msc)
{
	unsigned long flags;
	struct mpam_msc *msc = _msc;

	spin_lock_irqsave(&msc->hw_lock, flags);
	mpam_write_reg(msc, MPAMF_ECR, 1);
	spin_unlock_irqrestore(&msc->hw_lock, flags);
}

static void mpam_disable_msc_ecr(void *_msc)
{
	unsigned long flags;
	struct mpam_msc *msc = _msc;

	spin_lock_irqsave(&msc->hw_lock, flags);
	mpam_write_reg(msc, MPAMF_ECR, 0);
	spin_unlock_irqrestore(&msc->hw_lock, flags);
}
...
static irqreturn_t mpam_irq_handler(int irq, void *dev_id)
{
	...
	spin_lock(&msc->hw_lock);
	reg = mpam_msc_read_esr(msc);
	spin_unlock(&msc->hw_lock);
