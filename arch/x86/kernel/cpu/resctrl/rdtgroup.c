				...
				struct rdtgroup *rdtgrp)
{
	...
	if (ret) {
		...
		if (rdtgrp->type == RDTCTRL_GROUP) {
			...
			parent_closid = rdtgrp->mon.parent->closid;
...
static u32 resctrl_get_config_cdp(u32 closid, struct resctrl_schema *s,
				  struct rdt_domain *d)
{
	enum resctrl_conf_type t = s->conf_type;
	struct rdt_resource *r = s->res;
 ...
 * will be considered.
 *
 * Overlap test is not limited to the specific schema for which the CBM
 * is intended though - when dealing with CDP schema that share the
 * underlying hardware the overlap check should be performed on
 * the CDP schema sharing the hardware also.
 *
...
void resctrl_offline_domain(struct rdt_resource *r, struct rdt_domain *d)
{
	...
	if (r->mon_capable && resctrl_is_mbm_enabled())
		cancel_delayed_work(&d->mbm_over);
	if (resctrl_arch_is_llc_occupancy_enabled() && r->mon_capable &&
	    has_busy_rmid(r, d)) {
		/*
